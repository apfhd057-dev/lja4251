(() => {
  ...
  const mainpageSections = mainpageNotes
    .map((note) => document.getElementById(note.dataset.target))
    .filter(Boolean);
  ...
  const mainpageObserver = new IntersectionObserver(
    (entries) => {
      const active = entries
        .filter((entry) => entry.isIntersecting)
        .sort((a, b) => b.intersectionRatio - a.intersectionRatio)[0];
      ...
      if (!active) return;
      setActiveMainpageNote(active.target.id);
    },
    {
  ...
  mainpageSections.forEach((section) => mainpageObserver.observe(section));
